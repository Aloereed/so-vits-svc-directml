...
def run(rank, n_gpus, hps):
    ...
    net_g = SynthesizerTrn(
        ...
        **hps.model).to(dml)
    net_d = MultiPeriodDiscriminator(hps.model.use_spectral_norm).to(dml)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        g = spk.to(dml, non_blocking=True)
        spec, y = spec.to(dml, non_blocking=True), y.to(dml, non_blocking=True)
        c = c.to(dml, non_blocking=True)
        f0 = f0.to(dml, non_blocking=True)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, items in enumerate(eval_loader):
            c, f0, spec, y, spk = items
            g = spk[:1].to(dml)
            spec, y = spec[:1].to(dml), y[:1].to(dml)
            c = c[:1].to(dml)
            f0 = f0[:1].to(dml)
            mel = spec_to_mel_torch(
                spec,
